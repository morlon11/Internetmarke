...
async function testAuthService() {
    ...
    if (!getCredentials().client_id ||
        !getCredentials().client_secret ||
        !getCredentials().username ||
        !getCredentials().password) {
        console.error('Fehler: Die Authentifizierungsdaten fehlen in den Umgebungsvariablen. Bitte stellen Sie sicher, dass Ihre .env-Datei korrekt ist.');
        process.exit(1);
    }
    try {
